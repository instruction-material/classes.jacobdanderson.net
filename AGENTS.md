...
- Preserve learner-safe Scratch solution playback: only exact public Scratch project URLs may open, the player stays lazy and sandboxed in an accessible disposable dialog, and `/courses` alone receives the Scratch frame policy. Keep the neutral `/privacy` notice and never add Math-only interactives to this fork.
