# PyGames Expansion Audit Plan

## Current State Snapshot

- Course file: `front-end/src/stores/courses/pygames.ts`
- Mapped repository: `PyGames`
- Mapped repository link: https://github.com/instruction-material/PyGames
- Instruction-Material local inventory: 3 visible top-level directories, 45 top-level files
- Module count: 16
- Curriculum entry count: 72
- `projectLink` count: 0
- `solutionLink` count: 31
- Repo folder overlap: 0/3 (0%) of top-level repo folders appear in catalog links.
- Top-level repo folders detected from links: 31 across 1 repo(s)
- Representative linked folders: No overlap detected yet.

## Audit Findings

1. Catalog parity check indicates 16 module headers and 72 curriculum entries; confirm each has deliberate sequencing and completion criteria.
2. Starter/solution balance is uneven: `projectLink` count is 0 while `solutionLink` count is 31.
...
- Reconcile catalog modules against `PyGames` and confirm linkable starter/solution sources.
