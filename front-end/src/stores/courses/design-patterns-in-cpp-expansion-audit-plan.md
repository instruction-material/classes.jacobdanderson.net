# Design Patterns in C++ Expansion Audit Plan

## Current State Snapshot

- Course file: `front-end/src/stores/courses/design-patterns-in-cpp.ts`
- Mapped repository: `Design-Patterns-in-CPP`
- Mapped repository link: https://github.com/instruction-material/Design-Patterns-in-CPP
- Instruction-Material local inventory: 8 visible top-level directories, 2 top-level files
- Module count: 13
- Curriculum entry count: 52
- `projectLink` count: 1
- `solutionLink` count: 1
- Repo folder overlap: 0/8 (0%) of top-level repo folders appear in catalog links.
- Top-level repo folders detected from links: 0 across 0 repo(s)
- Representative linked folders: No overlap detected yet.

## Audit Findings

1. Catalog parity check indicates 13 module headers and 52 curriculum entries; confirm each has deliberate sequencing and completion criteria.
...
- Reconcile catalog modules against `Design-Patterns-in-CPP` and confirm linkable starter/solution sources.
...
- Add anti-pattern identification checkpoints before each code-first design exercise.
- Add two refactoring rescue labs that require choosing and applying alternatives.
- Add a design review rubric that includes trade-off analysis (readability, flexibility, complexity).
