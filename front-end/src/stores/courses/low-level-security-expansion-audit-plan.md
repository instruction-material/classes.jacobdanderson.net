# Low Level Security Expansion Audit Plan

## Current State Snapshot

- Course file: `front-end/src/stores/courses/low-level-security.ts`
- Mapped repository: `Low-Level-Security`
- Mapped repository link: https://github.com/instruction-material/Low-Level-Security
- Instruction-Material local inventory: 12 visible top-level directories, 2 top-level files
- Module count: 6
- Curriculum entry count: 36
- `projectLink` count: 6
- `solutionLink` count: 6
- Repo folder overlap: 0/12 (0%) of top-level repo folders appear in catalog links.
- Top-level repo folders detected from links: 0 across 0 repo(s)
- Representative linked folders: No overlap detected yet.

## Audit Findings

1. Catalog parity check indicates 6 module headers and 36 curriculum entries; confirm each has deliberate sequencing and completion criteria.
2. Starter/solution balance is uneven: `projectLink` count is 6 while `solutionLink` count is 6.
...
- Reconcile catalog modules against `Low-Level-Security` and confirm linkable starter/solution sources.
